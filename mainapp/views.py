...
def password_change(request, code):
	...
	try:
		profile = Patient.objects.get(verification_data__contains={"password_reset_code": code})
	except Patient.DoesNotExist:
		pass

	try:
		profile = Doctor.objects.get(verification_data__contains={"password_reset_code": code})
	except Doctor.DoesNotExist:
		pass

	try:
		profile = Nurse.objects.get(verification_data__contains={"password_reset_code": code})
	except Nurse.DoesNotExist:
		pass

	try:
		profile = Receptionist.objects.get(verification_data__contains={"password_reset_code": code})
	except Receptionist.DoesNotExist:
		pass

	if profile is None:
		messages.add_message(request,messages.SUCCESS, "Error. No account/profile exists.")
		return redirect('mainapp:login')
	...
	if request.method == 'POST' and 'CHANGE_PASSWORD' in request.POST:
		...
		if new_password != confirm_password:
			messages.add_message(request,messages.SUCCESS, "Password Mismatch. Try again.")
			return redirect('mainapp:password_change', code=code)

		if(len(new_password)<8 or any(letter.isalpha() for letter in new_password)==False or any(capital.isupper() for capital in new_password)==False or any(number.isdigit() for number in new_password)==False):
			messages.add_message(request,messages.SUCCESS, "Your password is not strong enough. Try again.")
			return redirect('mainapp:password_change', code=code)

		if 'password_reset_code' in profile.verification_data:
			del profile.verification_data['password_reset_code']
	...
	return render(request,"mainapp/changeUserPassword.html", {})

def forgetpassword(request):
	if request.method == 'POST' and 'FORGET_PASSWORD_REQUEST' in request.POST:
		...
		if user is not None:
			...
			try:
				profile = Receptionist.objects.get(user=user)
			except Receptionist.DoesNotExist:
				pass

			if profile is not None:
				token = str(uuid.uuid4())
				profile.verification_data["password_reset_code"] = token
				profile.save()
				link = "http://localhost:8000/password_change/{}/".format(token)

				email_subject = "Request to change AtEast Password"
				message = """
				Hi {},
					Please click the link below to change your account password.
					Link: {}

				Thanks,
				Management
				""".format(user.get_full_name(), link)
				email_message = EmailMessage(email_subject, message, settings.EMAIL_HOST_USER, [user.email])
				email_message.send()

				messages.add_message(request,messages.SUCCESS, "Check your email for a password change link.")
				return redirect('mainapp:forgetpassword')
...
def staticpages(request, page_name):
	if page_name == 'atEaseFeatures':
		return render(request,"mainapp/atEaseFeatures.html", {})
	elif page_name == 'accessibility':
		return render(request,"mainapp/accessibility.html", {})
	elif page_name == 'support':
		return render(request,"mainapp/support.html", {})
	elif page_name == 'termsOfUse':
		return render(request,"mainapp/termsOfUse.html", {})
	elif page_name == 'privacyPolicy':
		return render(request,"mainapp/privacyPolicy.html", {})
	elif page_name == 'commonSymptioms':
		return render(request,"mainapp/commonSymptioms.html", {"health_advice": HealthAdvice.objects.all()})
	else:
		return redirect('mainapp:mainpage')